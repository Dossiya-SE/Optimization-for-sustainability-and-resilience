...
# Example point strictly inside S₁
const xS1 = (2.0, 3.0)

# Displacement vector x-x⁰
const d = (
    xS1[1] - x0[1],
    xS1[2] - x0[2]
)
...
# A tangent direction to H is t=(1,-2)
const tH = (1.0, -2.0)

# a must be normal to H
...
const h_label = (
    h_anchor_x - 0.22 * unit_normal_H[1],
    h_anchor_y - 0.22 * unit_normal_H[2],
)
...
mathlabel!(ax, 2.65, 5.10, L"S_1:\;2x_1+x_2\geq4", S1_TEXT; fontsize = 16, halo = 4.2)
mathlabel!(ax, 0.20, 0.65, L"S_2:\;2x_1+x_2\leq4", S2_TEXT; fontsize = 16, halo = 4.2)
...
mathlabel!(ax, -0.17, 4.18, L"(0,4)", H_BLUE; fontsize = 13, align = (:right, :bottom), halo = 3.5)
mathlabel!(ax, 2.15, -0.18, L"(2,0)", H_BLUE; fontsize = 13, align = (:left, :center), halo = 3.5)

# ============================================================
# 15. REFERENCE POINT x⁰=(1,2)
# ============================================================
...
const x0_label = (
    x0[1] - 0.43 * unit_normal_H[1],
    x0[2] - 0.43 * unit_normal_H[2],
)

mathlabel!(ax, x0_label[1], x0_label[2], L"x^0=(1,2)", A_ORANGE; fontsize = 14, halo = 4.0)

# ============================================================
# 16. NORMAL VECTOR a=(2,1)
# ============================================================
...
const a_label = (
    mid_a[1] - 0.20 * normal_to_a[1],
    mid_a[2] - 0.20 * normal_to_a[2],
)
...
mathlabel!(ax, 2.06, 3.28, L"x=(2,3)", X_PURPLE; fontsize = 14, align = (:left, :center), halo = 4.0)

# ============================================================
# 18. DISPLACEMENT VECTOR x-x⁰=(1,1)
# ============================================================
...
const d_label = (
    mid_d[1] + 0.29 * normal_to_d[1],
    mid_d[2] + 0.29 * normal_to_d[2],
)
...
const theta_label = (
    x0[1] + 0.80 * cos(angle_mid),
    x0[2] + 0.80 * sin(angle_mid),
)

mathlabel!(ax, theta_label[1], theta_label[2], L"\theta<90^\circ", ANGLE_GREEN; fontsize = 13, halo = 3.8)
...
# ============================================================
# 21. COMPACT INTERPRETATION
# ============================================================

Label(
    fig[2, 1],
    L"a^\top(x-x^0)=3>0\;\Longrightarrow\;\theta<90^\circ";
    fontsize = 15,
    color = INK,
    padding = (4, 4, 8, 2),
)
...
save("IEE574_HW2_Problem2_complete.pdf", fig; pt_per_unit = 1)
save("IEE574_HW2_Problem2_complete.svg", fig; pt_per_unit = 1)
save("IEE574_HW2_Problem2_complete.png", fig; px_per_unit = 3)

println()
println("Problem 2 files created successfully:")
println("  IEE574_HW2_Problem2_complete.pdf")
println("  IEE574_HW2_Problem2_complete.svg")
println("  IEE574_HW2_Problem2_complete.png")
